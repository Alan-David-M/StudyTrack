 ...
 * - Imagen/PDF mediante Firebase Storage.
...
const ATTACHMENT_FOLDER = "taskAttachments";
...
async function saveTask(event) {
  ...
  try {
    ...
    if (state.selectedFile) {
      try {
        const uploadedAttachment = await uploadTaskAttachment(
          state.selectedFile,
          taskId
        );

        if (attachment?.path) {
          await safelyDeleteStorageFile(attachment.path);
        }
        ...
        showToast(
          ...
          "Firebase Storage rechazó el adjunto o todavía no está activado."
        );
      }
    } else if (
...
async function uploadTaskAttachment(file, taskId) {
  ...
  const path =
    `${ATTACHMENT_FOLDER}/${state.user.uid}/${taskId}/` +
    `${Date.now()}_${safeName}`;
  ...
  const snapshot = await new Promise((resolve, reject) => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      const timeoutError = new Error(
        "Firebase Storage no respondió. Comprueba que esté activado."
      );
      ...
      (progressSnapshot) => {
        const progress =
          (progressSnapshot.bytesTransferred /
            progressSnapshot.totalBytes) * 100;
...
async function safelyDeleteStorageFile(path) {
  if (!path) return;
  ...
  try {
    ...
    if (error?.code !== "storage/object-not-found") {
      console.warn("No se pudo borrar el archivo anterior:", error);
    }
  }
}
...
async function deleteSelectedTask() {
  ...
  try {
    if (task.attachment?.path) {
      await safelyDeleteStorageFile(task.attachment.path);
    }
...
function getFirebaseErrorMessage(error) {
  const messages = {
    ...
    "storage/unauthorized":
      "Firebase Storage rechazó el archivo. Revisa sus reglas.",
    "storage/object-not-found":
      "No encontramos el archivo solicitado.",
    "storage/quota-exceeded":
      "La cuota de Firebase Storage fue superada.",
    "storage/retry-limit-exceeded":
      "La subida tardó demasiado. Revisa tu conexión.",
    "storage/unknown":
      "Firebase Storage no está disponible o todavía no fue activado.",
    "storage/upload-timeout":
      "Firebase Storage no respondió. Actívalo y publica sus reglas antes de adjuntar archivos.",
    unavailable:
