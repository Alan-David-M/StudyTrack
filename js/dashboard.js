...
function calculateMetrics(subjects, tasks) {
    ...
    const pendingTasks = normalizedTasks.filter((task) => !task.completed);
    ...
    const upcomingTasks = pendingTasks
        .filter((task) => task.dueDate && task.dueDate >= startOfToday() && task.dueDate <= sevenDaysFromNow)
        .sort(sortByDueDate);
    ...
    const progress = normalizedTasks.length
        ? Math.round((completedTasks.length / normalizedTasks.length) * 100)
        : 0;
...
function normalizeTask(rawTask) {
    const status = String(rawTask.status || rawTask.state || "").toLowerCase();
    const completed =
    ...
    return {
        ...rawTask,
        title: rawTask.title || rawTask.name || rawTask.nombre || "Tarea sin título",
        subjectId: rawTask.subjectId || rawTask.materiaId || "",
        subjectName:
            rawTask.subjectName ||
            rawTask.materia ||
            rawTask.subject ||
        ...
        description:
            ...
            rawTask.descripcion ||
            "",
        priority: String(rawTask.priority || rawTask.prioridad || "media").toLowerCase(),
    ...
    if (!tasks.length) {
        elements.agendaList.innerHTML = `
            <div class="empty-state compact-empty">
                ...
                <button type="button" data-coming-soon="Tarea nueva">Agregar tarea</button>
            </div>
        `;
